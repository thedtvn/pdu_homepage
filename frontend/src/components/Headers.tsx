...
export default function Headers(props: { links: LinkObj[] }) {
    ...
    return (
        <>
            <Flex alignItems={"center"} justifyContent={"space-between"} bg="#000080" color="white" p={4}>
                <Link to={"/"}>
                    <Flex>
                        <Flex h={61} pl={1}>
            ...
            <Box position={"sticky"} top={0} zIndex={999} boxShadow={"0 0 10px 0 rgba(0, 0, 0, 0.5)"} >
                <Flex pl={5} pr={5} bg={"#900808"} display={{ md: "flex", base: "none" }} alignItems={"center"}>
                    <Link to={"/"}>
                        <IconButton
                            bg={"transparent"}
                            icon={<IoHomeSharp />}
                        ...
                        if (link.children) {
                            ...
                            return (
                                <React.Fragment key={link.name}>
                                    ...
                                    {isOpen && (
                                        <>
                                            <Box borderBottomRadius={10} mt={1} bg="rgba(213, 213, 213, 0.54)" backdropFilter={"blur(2px)"} border={"2px solid"} borderTop={"none"} p={1} ref={refs.setFloating} zIndex={999} style={floatingStyles}>
